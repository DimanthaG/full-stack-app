  ...
  callbacks: {
      ...
      if (account?.provider === "google" && profile) {
        try {
          ...
          if (!existingUser) {
            ...
            // If user has both password and Google auth, they're now merged
    ...
    async jwt({ token, user, account }) {
      if (user) {
        ...
        // Pass the provider information to the session
        if (account) {
          token.provider = account.provider;
        }
        if ((user as any).isMerged) {
          token.isMerged = true;
        }
      }
      return token;
